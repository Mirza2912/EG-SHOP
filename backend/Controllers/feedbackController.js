    ...
    try {
      ...
      if (req.user.role === 'admin') {
        ...
        if (!recipient || recipient.role === 'admin') {
          return res.status(400).json({ success: false, message: 'Invalid recipient or recipient is an admin' });
        }
      }
      ...
      if (req.user.role === 'admin') {
        ...
        if (!conversation) {
          ...
          if (!admin) {
            return res.status(404).json({ success: false, message: 'No admin found to start conversation' });
          }
          conversation = await Conversation.create({
      ...
      const recipient = (req.user.role === 'admin') ? recipientId : conversation.participants.find(participant => participant.toString() !== senderId.toString());
      ...
      if (req.user.role === 'admin') {
        if (recipientId) {
          ...
          const conversationPromises = conversations.map(async (conv) => {
            const convMessages = await Message.find({ conversationId: conv._id })
              .populate('sender', 'name email')
              .populate('recipient', 'name email')
              .sort('createdAt');
